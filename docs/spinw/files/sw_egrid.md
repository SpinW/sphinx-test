...
```{image} /_static/img/generated/sw__1.png
:alt: sw_plotspec(spectra,
```
...
: Input structure, contains spin-spin correlation functions. Supported
  inputs are produced by [spinw.spinwave](#spinw-spinwave), [spinw.powspec](#spinw-powspec) and
  [spinw.scga].
...
: Temperature, used to calculate the Bose factor in units
  depending on the Boltzmann constant stored in [spinw.unit](#spinw-unit). Default
...
[spinw.spinwave](#spinw-spinwave) \| [sw_neutron](#sw-neutron)

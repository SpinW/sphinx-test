...
spectrum that is calculated either by [spinw.spinwave](#spinw-spinwave) or
[spinw.powspec](#spinw-powspec). It can plot dispersion or intensities as line plots or
...
: Experimental data points to plot over the calculated spectrum.
  ...
  as the input file of [spinw.fitspec](#spinw-fitspec) see help for details on the file
  format), or it is a structure that contains the already imported data
  using the [sw_readtable](#sw-readtable) function, e.g.
...
[spinw.plot](#spinw-plot) \| [spinw.spinwave](#spinw-spinwave) \| [sw_surf] \| [sw_label]

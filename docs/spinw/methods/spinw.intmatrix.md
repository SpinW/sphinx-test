...
: If true each coupling is sorted for consistent plotting of
  ...
  points from `atom1` to `atom2`, for details see [spinw.coupling](#spinw-coupling).
...
: structure with fields `iso`, `ani`, `dm`, `gen`, `bq`, `dip` and
  `all`. It describes the bonds between spins. Every field is a matrix,
              ...
              [spinw.matom](#spinw-matom) list. The following rows contains the
...
[spinw.table](#spinw-table) \| [spinw.symop](#spinw-symop)

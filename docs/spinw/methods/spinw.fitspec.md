...
: Path to the file that stores the experimental data. For the
  input data format see [sw_readspec](#sw-readspec).
...
: String that determines the type of optimizer to use, possible values:
  * `'pso'`       Particle-swarm optimizer, see `ndbase.pso`,
                  default.
  * `'simplex'`   Matlab built-in simplex optimizer, see [fminsearch](https://www.mathworks.ch/help/matlab/ref/fminsearch.html).
...
`'hermit'`
: Method for matrix diagonalization, for details see [spinw.spinwave](#spinw-spinwave).
...
Any other option used by [spinw.spinwave](#spinw-spinwave) function are also accepted.
  
## See Also
  
[spinw.spinwave](#spinw-spinwave) \| [spinw.matparser](#spinw-matparser) \| [sw_egrid](#sw-egrid) \| [sw_neutron](#sw-neutron) \| [sw_readspec](#sw-readspec)

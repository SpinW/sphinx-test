...
details see the [sw_mex](#sw-mex) and [swpref.setpref](#swpref-setpref) functions.
...
```{image} /_static/img/generated/spinw_spi_1.png
...
: If true, the g-tensor will be included in the spin-spin correlation
  ...
  isotropic g-tensor is possible afterwards using the [sw_instrument](#sw-instrument)
  function.
...
: structure, with the following fields:
  ...
  * `V`       Transformation matrix from the normal magnon modes to the
              magnons localized on spins using the following: $x_i = \sum_j
              V_{ij} \times x_j'$. Only saved if `saveV` is true.
  ...
  * `obj`     The copy (clone) of the input `obj`, see [spinw.copy](#spinw-copy).
...
[spinw](#spinw) \| [spinw.spinwavesym](#spinw-spinwavesym) \| [sw_mex](#sw-mex) \| [spinw.powspec](#spinw-powspec) \| [sortmode]

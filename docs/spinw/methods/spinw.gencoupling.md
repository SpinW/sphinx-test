...
{% include warning.html content=" This function has to be used after the crystal structure is defined.
  The [spinw.addcoupling](#spinw-addcoupling) function will only work afterwards. " %}
 
## Examples
 
A triangular lattice is generated using `spinw.gencoupling` and
the [spinw.table](#spinw-table) function lists the 1st, 2nd and 3rd neighbor bonds:
...
: If `true`, equivalent bonds are always generated based on
  ...
  if they are given ([spinw.symmetry](#spinw-symmetry) returns `true`).
 
`'maxDistance'`
: Maximum bond length that will be stored in the
  [spinw.coupling](#spinw-coupling) property in units of Å. Default value is 8.
...
The [spinw.coupling](#spinw-coupling) field of `obj` will store the new bond list, while
overwriting previous bond list. This will also remove any previous
assignment of exchange matrices to bonds.
 
## See Also
 
[spinw](#spinw) \| [spinw.symmetry](#spinw-symmetry) \| [spinw.nosym](#spinw-nosym)

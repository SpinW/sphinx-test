...
[spinw.gencoupling](#spinw-gencoupling).
...
```{image} /_static/img/generated/spinw_addcou_1.png
:alt: plot(cryst,
```
...
: Contains labels of atoms (string) or index of atoms (integer) that is
  compared to [spinw.unit_cell](#spinw-unit-cell) where all symmetry inequivalent atoms are
...
The function adds extra entries to the [spinw.coupling](#spinw-coupling) property of
`obj`. Specifically it will modify `obj.coupling.mat_idx`,
`obj.coupling.type` and `obj.coupling.sym` matrices.
  
## See Also
  
[spinw](#spinw) \| [spinw.gencoupling](#spinw-gencoupling) \| [spinw.addmatrix](#spinw-addmatrix)

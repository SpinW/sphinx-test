...
`'keepq'` option to `false`. In the default case the [spinw.spinwave](#spinw-spinwave)
...
```{image} /_static/img/generated/spinw_ne_1.png
:alt: swplot.zoom(1.5)
```
```matlab
subplot(2,1,1)
...
```{image} /_static/img/generated/spinw_ne_2.png
:alt: sw_plotspec(sw_egrid(tri_orth.spinwave({[0 0 0] [1 1 0] 501})),
```
...
: If true, the reciprocal lattice units of the new model will be
  ...
  definition is invoked, such as in [spinw.spinwave](#spinw-spinwave). Default value is
  `false`.
...
[spinw.genlattice](#spinw-genlattice) \| [spinw.gencoupling](#spinw-gencoupling) \| [spinw.nosym](#spinw-nosym)
